...
# Written by Louis Cochrane: Lancaster University
# In Collaboration with Derek Gatherer: Lancaster University
# 2020
...
from dependencies.distanceMatrixEngine import generate_distance_matrix, generate_distance_matrix_non_family
from dependencies.matrixClass import Matrix
from dependencies.pydub import AudioSegment
from dependencies.dynamicProgrammingEngine import generate_dyanmic_programming_matrix, traceback, traceback_non_family
...
class Globals:
    ...
    familyDict = {'1': 'A', '2': 'A', '3': 'B', '4': 'B', '5': 'C', '6': 'C', '7': 'D', '8': 'D', '9': 'E', '10': 'E',
                  '11': 'F', '12': 'F', '13': 'G', '14': 'G', '15': 'H', '16': 'H', '17': 'I', '18': 'I', '19': 'J',
                  '20': 'J', '21': 'K', '22': 'K', '23': 'L', '24': 'L', '25': 'M', '26': 'M', '27': 'N', '28': 'N',
                  '29': 'O', '30': 'O', '31': 'P', '32': 'P', '1R': 'AR', '2R': 'AR', '3R': 'BR', '4R': 'BR',
                  '5R': 'CR', '6R': 'CR', '7R': 'DR', '8R': 'DR', '9R': 'ER', '10R': 'ER', '11R': 'FR', '12R': 'FR',
                  '13R': 'GR', '14R': 'GR', '15R': 'HR', '16R': 'HR', '17R': 'IR', '18R': 'IR', '19R': 'JR',
                  '20R': 'JR', '21R': 'KR', '22R': 'KR', '23R': 'LR', '24R': 'LR', '25R': 'MR', '26R': 'MR',
                  '27R': 'NR', '28R': 'NR', '29R': 'OR', '30R': 'OR', '31R': 'PR', '32R': 'PR'}

    reverseFamilyDict = {'-': ['-'], 'A': ['1', '2'], 'B': ['2', '3'], 'C': ['3', '4'], 'D': ['4', '5'],
                         'E': ['5', '6'], 'F': ['6', '7'], 'G': ['7', '8'], 'H': ['8', '9'], 'I': ['9', '10'],
                         'J': ['10', '11'], 'K': ['11', '12'], 'L': ['12', '13'], 'M': ['13', '14'], 'N': ['14', '15'],
                         'O': ['15', '16'], 'P': ['16', '17'], 'AR': ['1r', '2r'], 'BR': ['2r', '3r'],
                         'CR': ['3r', '4r'], 'DR': ['4r', '5r'], 'ER': ['5r', '6r'], 'FR': ['6r', '7r'],
                         'GR': ['7r', '8r'], 'HR': ['8r', '9r'], 'IR': ['9r', '10r'], 'JR': ['10r', '11r'],
                         'KR': ['11r', '12r'], 'LR': ['12r', '13r'], 'MR': ['13r', '14r'], 'NR': ['14r', '15r'],
                         'OR': ['15r', '16r'], 'PR': ['16r', '17r']}
...
def get_length_and_note():
    ...
    while not sequenceLength.isdigit():
        newLine()
...
def random_dual_scambi_non_family():
    # First we generate two mono sequences, using the core generator.

    # TODO: Add the abilty for the two strands to be different lengths.
    #       can probably do this just feeding in two different numbers to the core sequence generator.
    #       May need to build a new 'get_length_and_note to grab two different inputs though.
    #       Or just override in this particular function and grab manually.

    sequenceLength, firstNote = get_length_and_note()

    if firstNote == "r":
        firstStart = random.choice(list(Globals.startDict.keys()))
        firstNote = Globals.startDict[firstStart][random.randint(0, len(Globals.startDict[firstStart]) - 1)]
    ...
    startnote = leftChannel[0]
    endnote = endNote

    leftOutput = leftChannel[1:]
    leftOutput.insert(0, startnote)
    leftOutput.append(endnote)

    rightOutput = rightChannel[1:]
    rightOutput.insert(0, startnote)
    rightOutput.append(endnote)
...
def build_from_alignment_non_family(leftChannelList, rightChannelList):
    # normalMode()

    leftChannelSequences = leftChannelList
    rightChannelSequences = rightChannelList
...
def main():
    ...
    while programChoice != "11":
        ...
        newLine()
        printASCI()
        print(" By Louis Cochrane: Derek Gatherer")
        print(" Lancaster University ")
        print("--------------------------------------------------")
        print(" [0] Run Full Sequence (Default)")
        print("[11] Quit")
        print("--------------------------------------------------")
        ...
        print(" [9] Run Full Family Alignment Sequence")
        print("[10] Build Sound Files from Stored Alignment")
        print("--------------------------------------------------")
        ...
        if programChoice == "1":
            ...
            if not Globals.lastGeneratedSequence:
                newLine()
                print(
                    "No mono chain found: use the mono scambi generator to create a chain to build, or"
                    " enter a chain manually.")
            ...
            if Globals.lastGeneratedPairwiseMismatchTable == []:
                ...
                Globals.lastGeneratedDynamicProgrammingMatrix, Globals.rawMatrix, Globals.gapScore = generate_dyanmic_programming_matrix(
                    Globals.lastGeneratedPairwiseMismatchTable)
            ...
            if Globals.lastGeneratedPairwiseMismatchTable == [] or Globals.lastGeneratedDynamicProgrammingMatrix == [] or Globals.gapScore == None:
                ...
                newLine()
                print(
                    "Error: Necessary matrices and gap score missing, try running the full alignment sequence instead.")
                newLine()
            else:

                Globals.leftChannel, Globals.rightChannel = traceback(Globals.lastGeneratedDynamicProgrammingMatrix,
                                                                      Globals.rawMatrix, Globals.gapScore,
                                                                      Globals.lastGeneratedPairwiseMismatchTable)
...
def updated_sequence():
    newLine()

    manualOrAutomatic = input("Manual Mode [M]\nTo input your own Scambi Chain.\n\n"
                              "OR\n\nAutomatic mode [A]\nTo automatically build a chain according to Pousseur's rules.\n\n")

    if manualOrAutomatic.lower() == "m":
        manual = True
    else:
        manual = False

    if manual:
        Globals.lastGeneratedPairwiseMismatchTable = generate_distance_matrix_non_family(None, None, True)

    else:
        leftChannel, rightChannel = random_dual_scambi_non_family()
        Globals.lastGeneratedPairwiseMismatchTable = generate_distance_matrix_non_family(leftChannel, rightChannel,
                                                                                         False
    ...
    Globals.leftChannel, Globals.rightChannel = traceback_non_family(Globals.lastGeneratedDynamicProgrammingMatrix,
                                                                     Globals.rawMatrix, Globals.gapScore,
                                                                     Globals.lastGeneratedPairwiseMismatchTable)
    ...
    if Globals.leftChannel == [] or Globals.rightChannel == []:
        ...
        if choice == "y" or choice == "Y":
            build_from_alignment_non_family(Globals.leftChannel, Globals.rightChannel)
            newLine()
            print("Build complete!")
            print("Sound files exported to Alignment (Left/Right) Channel.wav")

        else:
            print("No sound files built - returning to menu.")
